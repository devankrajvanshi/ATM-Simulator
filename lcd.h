...
void state_to_string(unsigned int state[],unsigned char *state_str); //convert integer array to string array
...
/**********************************************************
  state_to_string(<integer_array>,<string_ptr>): 
	Converts integer array to string array with each string of length 2
***********************************************************/	
void state_to_string(unsigned int state[4],unsigned char state_str[4][3])
{	
	int i; // can't define inside for loop
	for(i=0;i<4;i++)
	{
		state_str[i][0]=48+(state[i]/10);
		state_str[i][1]=48+(state[i]%10);
	}
}
